...
async def mark_task_as_done(task_id: int, db: done_crud.AsyncSession = Depends(get_db)):
...
async def unmark_task_as_done(task_id: int, db: done_crud.AsyncSession = Depends(get_db)):
    done = await done_crud.get_done(db, task_id=task_id)
    if done is None:
